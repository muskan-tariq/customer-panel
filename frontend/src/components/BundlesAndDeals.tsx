...
const BundlesAndDeals = () => {
  const { products, loading, error } = useProducts('bundles');
    ...
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-medium mb-2">Special Bundles & Deals</h2>
            <p className="text-gray-500">Save more with our curated sets</p>
          </div>
          <Link 
            to="/products/bundles"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {displayProducts.map((product) => (
            <Link 
              key={product._id} 
              to={`/products/bundles/${product._id}`}
              className="group hover:shadow-lg transition-shadow duration-300 bg-white rounded-lg overflow-hidden"
            >
